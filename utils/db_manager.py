...
class DBManager:
    ...
    def get_stock_name_by_date(self, code: str, target_date: str) -> Optional[str]:
        """
        按指定日期查询股票当时的名称（用于回测还原历史ST状态）

        优先从 namechange 表查询名称变更历史，找到 target_date 生效的名称；
        若无变更记录，回退到 stock_basic 表的当前名称。

        Args:
            code: 股票代码（无后缀，如 000001）
            target_date: 目标日期，格式 YYYY-MM-DD 或 YYYYMMDD

        Returns:
            str: 当时的股票名称，查询失败返回 None
        """
        try:
            # 统一日期格式为 YYYYMMDD
            date_str = target_date.replace("-", "") if target_date else ""
            if not date_str:
                return None

            # 从 namechange 表查询：start_date <= target_date 且 (end_date >= target_date 或 end_date 为空)
            sql = """
                SELECT name FROM namechange
                WHERE (code = ? OR ts_code LIKE ?)
                  AND start_date <= ?
                  AND (end_date IS NULL OR end_date = '' OR end_date >= ?)
                ORDER BY start_date DESC
                LIMIT 1
            """
            results = self.query(sql, (code, f"{code}.%", date_str, date_str))
            if results:
                name = results[0].get('name')
                if name:
                    return name

            # namechange 无记录，回退到 stock_basic 当前名称
            sql_basic = "SELECT name FROM stock_basic WHERE code = ? LIMIT 1"
            results_basic = self.query(sql_basic, (code,))
            if results_basic:
                return results_basic[0].get('name')

            return None
        except Exception as e:
            logger.debug(f"按日期查询股票名称失败 {code} @ {target_date}: {str(e)}")
            return None

    def batch_get_stock_names_by_date(self, codes: List[str], target_date: str) -> Dict[str, str]:
        """
        批量按日期查询股票当时的名称
        ...
        result = {}
        for code in codes:
            name = self.get_stock_name_by_date(code, target_date)
            if name:
                result[code] = name
        return result

    def save_namechange_records(self, records: List[Dict]) -> int:
        """
        保存名称变更记录到 namechange 表（批量 upsert）

        Args:
            records: 名称变更记录列表，每条包含 ts_code/code/name/start_date/end_date/change_reason
        ...
        try:
            sql = """
                INSERT OR REPLACE INTO namechange (ts_code, code, name, start_date, end_date, change_reason)
                VALUES (?, ?, ?, ?, ?, ?)
            """
            count = 0
            for r in records:
                ...
                # 从 ts_code 提取无后缀代码
                code = r.get('code') or (ts_code.split('.')[0] if '.' in ts_code else ts_code)
                self.execute(sql, (
                    ts_code,
                    code,
                    r.get('name', ''),
            ...
            logger.error(f"保存名称变更记录失败: {str(e)}")
            return 0
